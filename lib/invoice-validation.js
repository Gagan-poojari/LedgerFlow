...
export async function runInvoiceValidation(invoice, options = {}) {
  ...
  if (gstin) {
    ...
    if (!gstResult.valid) {
      // OCR can misread one character of a 15-char GSTIN, producing a bad
      // checksum. Treat checksum failures as warnings so the user can
      // correct the field manually without being fully blocked.
      // Hard format errors (wrong length, illegal characters) stay as errors.
      if (gstResult.error?.toLowerCase().includes("checksum")) {
        warnings.push(
          `GSTIN checksum mismatch (${gstin}) — verify and correct if needed`
        );
      } else {
