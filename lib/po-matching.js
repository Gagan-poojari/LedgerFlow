...
/** Case-insensitive PO lookup with trimmed number. */
async function findPurchaseOrder(poNumber) {
  const normalized = poNumber?.trim();
  if (!normalized) return null;

  const exact = await PurchaseOrder.findOne({ poNumber: normalized }).lean();
  if (exact) return exact;

  return PurchaseOrder.findOne({
    poNumber: { $regex: new RegExp(`^${escapeRegex(normalized)}$`, "i") },
  }).lean();
}

async function findGrn({ grnNumber, poNumber }) {
  if (grnNumber?.trim()) {
    const n = grnNumber.trim();
    const exact = await GRN.findOne({ grnNumber: n }).lean();
    if (exact) return exact;
    return GRN.findOne({
      grnNumber: { $regex: new RegExp(`^${escapeRegex(n)}$`, "i") },
    }).lean();
  }
  ...
  if (!po) return null;
  return GRN.findOne({
    poNumber: { $regex: new RegExp(`^${escapeRegex(po)}$`, "i") },
  })
    .sort({ receivedDate: -1 })
    .lean();
}
...
export async function runPoMatch(invoice) {
  ...
  let grn = null;
  if (grnNumber) {
    grn = await findGrn({ grnNumber });
    if (!grn) {
      exceptions.push(`GRN ${grnNumber} not found`);
    }
  } else {
    grn = await findGrn({ poNumber });
  }
